...
function createObj() {
    ...
    data['isRate'] = ($('#obj-israte:checked').val() == 'on' ? true : false);
    ...
    $.ajax({
        ...
        success: function(response) {                    	
            if(response.indexOf($('#insertedx').val()) !== -1) {
            	...
            	// Notification
                toastr["success"]("Framework parameter type created successfully!", "Created");
            }
            else
                toastr["error"]("Item creation failed, record exists!", "Failure");
...
function editObj() {
    ...
    data['isRate'] = ($('#obj-eisrate:checked').val() == 'on' ? true : false);
    ...
    $.ajax({
        ...
        success: function(response) {                    	
            if(response.indexOf($('#updatedx').val()) !== -1) {
            	...
            	// Notification
                toastr["success"]("Framework parameter type updated successfully!", "Updated");
            }
            else
    ...
    $.ajax({
        ...
        success: function(response) {
        	...
        	if(response.length > 0) {
        		...
        		table += "<tr colspan='5'><td>No framework parameter type created yet.</td></tr>";
	...
	$.ajax({
        ...
        success: function(response) {
        	$.each(response, function(index, element) {
        		...
        		if(element[2] === true)
        			$('#obj-eisrate').prop('checked', true);
        		else
        			$('#obj-eisrate').prop('checked', false);
            });
