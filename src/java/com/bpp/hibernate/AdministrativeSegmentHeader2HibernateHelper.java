 ...
 * @author Adewale
 */
public class AdministrativeSegmentHeader2HibernateHelper {

    /* AdministrativeSegment methods begin */
    public synchronized String insert(AdministrativeSegmentHeader2 administrativeSegment) {
        AdministrativeSegmentHeader2 checkAdministrativeSegment = exists(administrativeSegment.getName());
    ...
    public synchronized String update(AdministrativeSegmentHeader2 administrativeSegment) {
    ...
    public synchronized String delete(AdministrativeSegmentHeader2 administrativeSegment) {
        AdministrativeSegmentHeader2 checkAdministrativeSegment = fetchOne(administrativeSegment.getId()+"");
        if (checkAdministrativeSegment != null) {
            final Session session = HibernateUtil.getSessionFactory().getCurrentSession();
            Transaction tx = null;
            try {
    ...
    public synchronized AdministrativeSegmentHeader2 exists(String name) {
        AdministrativeSegmentHeader2 administrativeSegment = null;
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            Query q = session.createQuery("from AdministrativeSegmentHeader2 as a where a.name='" + name + "'");
            //q.setMaxResults(2);
            administrativeSegment = (AdministrativeSegmentHeader2) q.uniqueResult();
            //session.getTransaction().commit();
    ...
    public synchronized String fetchAll() {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select * from Administrative_Segment_Header2 a where a.name<>'' order by a.name";
...
//            administrativeSegmentsList =  q.list();
   ...
   public synchronized String fetchAll(Integer parent) {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select * from Administrative_Segment_Header2 a where a.parent = "+parent+"order by a.name";
            //sql = "select productid from stockin";
            SQLQuery q = session.createSQLQuery(sql);
            administrativeSegmentsList = q.list();
//            Query q = session.createQuery("from AdministrativeSegment as a where a.name<>'' order by a.name");
//            administrativeSegmentsList =  q.list();
    ...
    public synchronized AdministrativeSegmentHeader2 fetchOne(String id){
        ...
        try {
            tx = session.beginTransaction();
            Query q = session.createQuery("from AdministrativeSegmentHeader2 as a where a.id='" + id + "'");
            administrativeSegment = (AdministrativeSegmentHeader2) q.uniqueResult();
            tx.commit();
        } catch (HibernateException e) {
            if (tx != null) {
    ...
    public synchronized String fetchOneJSON(String id) {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select a.id,a.name,a.code,a.parent from Administrative_Segment_Header2 a where a.id = "+id+"  order by a.id";
            //sql = "select productid from stockin";
