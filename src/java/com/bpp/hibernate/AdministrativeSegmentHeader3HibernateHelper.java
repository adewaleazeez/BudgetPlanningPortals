 ...
 * @author Adewale
 */
public class AdministrativeSegmentHeader3HibernateHelper {

    /* AdministrativeSegment methods begin */
    public synchronized String insert(AdministrativeSegmentHeader3 administrativeSegment) {
        AdministrativeSegmentHeader3 checkAdministrativeSegment = exists(administrativeSegment.getName());
    ...
    public synchronized String update(AdministrativeSegmentHeader3 administrativeSegment) {
    ...
    public synchronized String delete(AdministrativeSegmentHeader3 administrativeSegment) {
        AdministrativeSegmentHeader3 checkAdministrativeSegment = fetchOne(administrativeSegment.getId()+"");
        if (checkAdministrativeSegment != null) {
            final Session session = HibernateUtil.getSessionFactory().getCurrentSession();
            Transaction tx = null;
            try {
            ...
            System.out.println("No administrativeSegment 3 in Helper");
            return Utility.ActionResponse.NO_RECORD.toString();
        }
    }

    public synchronized AdministrativeSegmentHeader3 exists(String name) {
        AdministrativeSegmentHeader3 administrativeSegment = null;
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            Query q = session.createQuery("from AdministrativeSegmentHeader3 as a where a.name='" + name + "'");
            //q.setMaxResults(1);
            administrativeSegment = (AdministrativeSegmentHeader3) q.uniqueResult();
            //session.getTransaction().commit();
    ...
    public synchronized String fetchAll() {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select * from Administrative_Segment_Header3 a where a.name<>'' order by a.name";
...
//            administrativeSegmentsList =  q.list();
   ...
   public synchronized String fetchAll(Integer parent) {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select * from Administrative_Segment_Header3 a where a.parent = "+parent+"order by a.name";
            //sql = "select productid from stockin";
            SQLQuery q = session.createSQLQuery(sql);
            administrativeSegmentsList = q.list();
//            Query q = session.createQuery("from AdministrativeSegment as a where a.name<>'' order by a.name");
//            administrativeSegmentsList =  q.list();
    ...
    public synchronized AdministrativeSegmentHeader3 fetchOne(String id){
        ...
        try {
            tx = session.beginTransaction();
            Query q = session.createQuery("from AdministrativeSegmentHeader3 as a where a.id='" + id + "'");
            administrativeSegment = (AdministrativeSegmentHeader3) q.uniqueResult();
            tx.commit();
        } catch (HibernateException e) {
            if (tx != null) {
    ...
    public synchronized String fetchOneJSON(String id) {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select a.id,a.name,a.code,a.parent from Administrative_Segment_Header3 a where a.id = "+id+"  order by a.id";
            //sql = "select productid from stockin";
