 ...
 * @author Adewale
 */
public class EconomicSegmentHeader4HibernateHelper {

    /* EconomicSegment methods begin */
    public synchronized String insert(EconomicSegmentHeader4 economicSegment) {
        EconomicSegmentHeader4 checkEconomicSegment = exists(economicSegment.getName());
    ...
    public synchronized String update(EconomicSegmentHeader4 economicSegment) {
    ...
    public synchronized String delete(EconomicSegmentHeader4 economicSegment) {
        EconomicSegmentHeader4 checkEconomicSegment = fetchOne(economicSegment.getId() + "");
        if (checkEconomicSegment != null) {
            final Session session = HibernateUtil.getSessionFactory().getCurrentSession();
            Transaction tx = null;
            try {
    ...
    public synchronized EconomicSegmentHeader4 exists(String name) {
        EconomicSegmentHeader4 economicSegment = null;
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            Query q = session.createQuery("from EconomicSegmentHeader4 as a where a.name='" + name + "'");
            //q.setMaxResults(1);
            economicSegment = (EconomicSegmentHeader4) q.uniqueResult();
            //session.getTransaction().commit();
    ...
    public synchronized String fetchAll() {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select * from Economic_Segment_Header4 a where a.name<>'' order by a.id";
...
//            economicSegmentsList =  q.list();
    ...
    public synchronized List fetchAllAsList(String code) {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            //String sql = "select * from Economic_Segment a where a.name<>'' order by a.id";
            String sql = "select * from Economic_Segment_Header4 a where a.code LIKE '"+ code +"%' order by a.id";
            
            SQLQuery q = session.createSQLQuery(sql).addEntity(EconomicSegmentHeader4.class);
            economicSegmentsList = q.list();
    ...
    public synchronized String fetchAll(Integer parent) {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select * from Economic_Segment_Header4 a where a.parent = " + parent + "order by a.name";
            //sql = "select productid from stockin";
            SQLQuery q = session.createSQLQuery(sql);
            economicSegmentsList = q.list();
...
//            economicSegmentsList =  q.list();
    ...
    public synchronized EconomicSegmentHeader4 fetchOne(String id) {
        ...
        try {
            tx = session.beginTransaction();
            Query q = session.createQuery("from EconomicSegmentHeader4 as a where a.id='" + id + "'");
            economicSegment = (EconomicSegmentHeader4) q.uniqueResult();
            tx.commit();
        } catch (HibernateException e) {
            if (tx != null) {
    ...
    public synchronized String fetchOneJSON(String id) {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select a.id,a.name,a.code,a.parent from Economic_Segment_Header4 a where a.id = " + id + "  order by a.id";
            //sql = "select productid from stockin";
