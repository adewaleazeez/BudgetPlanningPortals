 ...
 * @author Adewale
 */
public class RolesHibernateHelper {

	public static final String TABLE_NAME = "Roles";
	public static final String RAW_TABLE_NAME = "Roles";

	/* Roles methods begin */
    public synchronized String insert(Roles role) {
        Roles checkRole = exists(role.getName());
        if (checkRole == null) {
    ...
    public synchronized String update(Roles role) {
        Roles checkRole = exists(role.getId());
        if (checkRole != null) {
            final Session session = HibernateUtil.getSessionFactory().getCurrentSession();
            Transaction tx = null;
            try {
    ...
    public synchronized String delete(Roles role) {
        Roles checkRole = exists(role.getName());
        if (checkRole != null) {
            final Session session = HibernateUtil.getSessionFactory().getCurrentSession();
            Transaction tx = null;
    ...
    public synchronized Roles exists(String name) {
        Roles role = null;
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            Query q = session.createQuery("from Roles as a where a.name='" + name + "'");
            //q.setMaxResults(1);
    ...
    public synchronized Roles exists(int id) {
        Roles btt = null;
    ...
    public synchronized Roles fetchObj(int id) {
        Roles obj = null;
    ...
    public synchronized String fetchAll() {
        ...
        try {
            ...
            //session.beginTransaction();
            tx = session.beginTransaction();
            String sql = "select * from Roles a where a.name<>'' order by a.name";
            //sql = "select productid from stockin";
            SQLQuery q = session.createSQLQuery(sql);
            rolesList = q.list();
//            Query q = session.createQuery("from Roles as a where a.name<>'' order by a.name");
//            rolesList =  q.list();
            //session.getTransaction().commit();
            Gson gson = new Gson();
            jsonList = gson.toJson(rolesList);
            tx.commit();
        } catch (HibernateException e) {
            if (tx != null) {
                tx.rollback();
