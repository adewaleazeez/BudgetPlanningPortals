...
import com.bpp.hibernate.ProgrammeHibernateHelper;
import com.bpp.hibernate.Programmes;
...
public class ProgrammeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ProgrammeHibernateHelper helper = new ProgrammeHibernateHelper();
            ...
            if (option.equals(Utility.OPTION_INSERT)) {
                synchronized (this) {
                    ...
                    programmes.setId(Integer.parseInt(helper.getMaxSerialNo("Programmes")) + 1);
                    programmes.setDatecreated(utility.dbDateNow());
                    programmes.setName(name);
                    programmes.setCode(code);
                    programmes.setOrgId(1);
                    resp = helper.insert(programmes);
            ...
            if (option.equals(Utility.OPTION_UPDATE)) {
                synchronized (this) {
                    Gson gson = new Gson();
                    programmes = helper.exists(Integer.parseInt(id));
                    if(programmes != null){
                        programmes.setName(name);
                        programmes.setCode(code);
                        resp = helper.update(programmes);
                        resp = gson.toJson(resp);
                    }else{
