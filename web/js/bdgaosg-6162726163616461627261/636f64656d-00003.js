...
function createObj() {
    ...
    $.ajax({
        type: "POST",
        url: $('#site-url').val() + "/BudgetStatusServlet",
        data: data,
        dataType: "text",
        cache: false,
        success: function(response) {                    	
            if(response.indexOf($('#insertedx').val()) !== -1) {
            	...
            	// Notification
                toastr["success"]("Budget status created successfully!", "Created");
            }
            else
            	// Notification
                toastr["error"]("Budget status creation failed!", "Failure");
    ...
    $.ajax({
        ...
        success: function(response) {                    	
            if(response.indexOf($('#updatedx').val()) !== -1) {
            	...
            	// Notification
                toastr["success"]("Budget status updated successfully!", "Updated");
            }
            else
    ...
    $.ajax({
        ...
        success: function(response) {
        	...
        	if(response.length > 0) {
        		...
        		table += "<tr colspan='5'><td>No budget status created yet.</td></tr>";
